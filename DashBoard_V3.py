...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Store connection ID for 1D plot mouse motion event
        self._1d_mouse_motion_cid = None 
    ...
    def init_ui(self):
        ...
        # Connect mouse motion events for all 2D plots to update coordinates display
        self.data_plotter_2d_main.canvas.mpl_connect('motion_notify_event', self.on_main_2d_plot_mouse_move)
        self.data_plotter_2d_x_cut.canvas.mpl_connect('motion_notify_event', self.on_x_cut_plot_mouse_move)
        self.data_plotter_2d_y_cut.canvas.mpl_connect('motion_notify_event', self.on_y_cut_plot_mouse_move)
        ...
        # Removed the caption label as requested
        ...
        # Add X,Y,Z data show window for plot hover pointer
        self.mouse_coords_display_2d = QLabel("X: -, Y: -, Z: -") # Updated for Z data
        self.mouse_coords_display_2d.setAlignment(Qt.AlignCenter)
        self.mouse_coords_display_2d.setStyleSheet("font-weight: bold; padding: 5px; border: 1px solid #ccc; border-radius: 5px;")
        _2d_controls_layout.addWidget(self.mouse_coords_display_2d)
        ...
        # Add X,Y data show window for 1D plot hover pointer
        # self.mouse_coords_display_1d = QLabel("X: -, Y: -")
        # self.mouse_coords_display_1d.setAlignment(Qt.AlignCenter)
        # self.mouse_coords_display_1d.setStyleSheet("font-weight: bold; padding: 5px; border: 1px solid #ccc; border-radius: 5px;")
        # _1d_plot_and_toolbar_layout.addWidget(self.mouse_coords_display_1d)

        # Initial connection for the 1D plot's mouse motion event.
        # This connection will be updated later in update_1d_plot_from_column_selection
        # whenever a new plot is drawn.
        # self._1d_mouse_motion_cid = self.data_plotter_1d.canvas.mpl_connect('motion_notify_event', self.on_1d_plot_mouse_move)
    ...
    def _load_and_plot_2d(self, file_path):
        try:
            ...
            self.data_plotter_2d_main.plot_image(self.data_2d_loaded,
                                                 ...
                                                 cbar_label="Intensity") # Default label after removal
    ...
    def update_1d_plot_from_column_selection(self):
        ...
        if self.current_1d_df is None:
            self.data_plotter_1d.clear_plot()
            self.data_1d_display.clear()
            # Disconnect the old event listener if it exists and the plot is cleared
            if self._1d_mouse_motion_cid is not None and self.data_plotter_1d.canvas is not None:
                try:
                    self.data_plotter_1d.canvas.mpl_disconnect(self._1d_mouse_motion_cid)
                except TypeError:
                    pass # Already disconnected or invalid CID
                self._1d_mouse_motion_cid = None
            return
        
        # Disconnect the old event listener before creating a new plot
        if self._1d_mouse_motion_cid is not None and self.data_plotter_1d.canvas is not None:
            try:
                self.data_plotter_1d.canvas.mpl_disconnect(self._1d_mouse_motion_cid)
            except TypeError:
                pass # Already disconnected or invalid CID
            self._1d_mouse_motion_cid = None # Reset the CID
        ...
        if series_to_plot:
            ...
            # Reconnect the mouse motion event after the plot is drawn and new axes are created
            # self._1d_mouse_motion_cid = self.data_plotter_1d.canvas.mpl_connect('motion_notify_event', self.on_1d_plot_mouse_move)
    ...
    def update_2d_cuts(self):
        ...
        if self.data_2d_loaded is not None and self.x_coords_2d is not None and self.y_coords_2d is not None:
            ...
            self.data_plotter_2d_x_cut.plot_line(x_series_data, # Pass the list
                                                 ...
                                                 ylabel="Intensity") # Use a default label for value axis

            # Display X-cut data
            x_cut_df = pd.DataFrame({
                self.plot_2d_xaxis_input.text(): self.x_coords_2d,
                "Intensity": x_cut_data_y # Use a default label for value axis
            })
            ...
            y_cut_df = pd.DataFrame({
                ...
                "Intensity": y_cut_data_x # Use a default label for value axis
            })
    ...
    def set_2d_cbar_label(self, label_text):
        # This function is no longer needed as the input has been removed
        pass

    def on_main_2d_plot_mouse_move(self, event):
        """Updates the X,Y,Z display label with the mouse coordinates when hovering over the main 2D plot."""
        if event.inaxes == self.data_plotter_2d_main.ax and event.xdata is not None and event.ydata is not None:
            if self.data_2d_loaded is not None and self.x_coords_2d is not None and self.y_coords_2d is not None:
                # Find the closest indices in the data array
                x_idx = np.argmin(np.abs(self.x_coords_2d - event.xdata))
                y_idx = np.argmin(np.abs(self.y_coords_2d - event.ydata))

                # Ensure indices are within bounds
                if 0 <= y_idx < self.data_2d_loaded.shape[0] and 0 <= x_idx < self.data_2d_loaded.shape[1]:
                    z_value = self.data_2d_loaded[y_idx, x_idx]
                    self.mouse_coords_display_2d.setText(f"Main 2D - X: {event.xdata:.2f}, Y: {event.ydata:.2f}, Z: {z_value:.2f}")
                else:
                    self.mouse_coords_display_2d.setText("Main 2D - X: -, Y: -, Z: - (Out of bounds)")
            else:
                self.mouse_coords_display_2d.setText("Main 2D - X: -, Y: -, Z: - (No data loaded)")
        else:
            # Clear or reset if mouse leaves the main 2D plot
            if self.mouse_coords_display_2d.text().startswith("Main 2D"):
                self.mouse_coords_display_2d.setText("X: -, Y: -, Z: -")

    def on_x_cut_plot_mouse_move(self, event):
        """Updates the X,Z display label when hovering over the X-cut plot."""
        if event.inaxes == self.data_plotter_2d_x_cut.ax and event.xdata is not None and event.ydata is not None:
            self.mouse_coords_display_2d.setText(f"X-Cut - X: {event.xdata:.2f}, Intensity: {event.ydata:.2f}")
        else:
            # Clear or reset if mouse leaves the X-cut plot
            if self.mouse_coords_display_2d.text().startswith("X-Cut"):
                self.mouse_coords_display_2d.setText("X: -, Y: -, Z: -")

    def on_y_cut_plot_mouse_move(self, event):
        """Updates the Y,Z display label when hovering over the Y-cut plot."""
        if event.inaxes == self.data_plotter_2d_y_cut.ax and event.xdata is not None and event.ydata is not None:
            # For Y-cut, the X-axis is actually the Y-coordinate from the original 2D data
            self.mouse_coords_display_2d.setText(f"Y-Cut - Y: {event.xdata:.2f}, Intensity: {event.ydata:.2f}")
        else:
            # Clear or reset if mouse leaves the Y-cut plot
            if self.mouse_coords_display_2d.text().startswith("Y-Cut"):
                self.mouse_coords_display_2d.setText("X: -, Y: -, Z: -")

    # def on_1d_plot_mouse_move(self, event):
    #     """Updates the X,Y display label with the mouse coordinates when hovering over the 1D plot."""
    #     if event.inaxes == self.data_plotter_1d.ax and event.xdata is not None and event.ydata is not None:
    #         self.mouse_coords_display_1d.setText(f"X: {event.xdata:.2f}, Y: {event.ydata:.2f}")
    #     else:
    #         # Clear or reset if mouse leaves the 1D plot
    #         if self.mouse_coords_display_1d.text().startswith("X:"):
    #             self.mouse_coords_display_1d.setText("X: -, Y: -")
